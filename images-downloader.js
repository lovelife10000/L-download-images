...
const images = (images, path,filename1) => {
	return new Promise((resolve, reject) => {
		return initChecking(images, path,filename1).then(images => {
			...
			.then(imagesReadyToDl => { return Promise.all(_.map(imagesReadyToDl, (fileToDl, key) => { return downloadFile(fileToDl, key, path,filename1)}))})
